...
float calculate_pi(int i, int k, float y[])
{
    ...
    n[0]=i;
    pi = yn_n(n, y);
//    printf("alpha(%d)[%d,%d] = %f\n",k, n[0], n[1], y[pos(n)]/factorial(k));
    while(n[N-1] <= k/2+k%2)
    {
        flag = true;
        for(j=1;flag && j<N;j++)
...
void print_result(int k, float y[])
{
    int i;
    for(i=0; i <= k/2+k%2; i++)
        printf("Pi(%d)[%d] = %f\n", k, i, calculate_pi(i,k,y)/factorial(k));
    printf("\n");
    
    FILE* data = fopen("QueueLocalChoice_Expansion.txt", "a");
    for(i=0; i <= k/2+k%2; i++)
...
int main(void)
{
    ...
    FILE* data = fopen("QueueLocalChoice_Expansion.txt", "w");
    fprintf(data, "N	%d\n\n", N);
    fclose(data);
