...
int run_detailed_system(int n, float rho, int steps, int samples)
{
    ...
    FILE* data = fopen("QueueGeneralChoice_Detailed.txt", "a");
    fprintf(data, "\nN	%d\nrho	%f\nSteps	%.0e\nSample	%.0e\n\n", n, rho, (double)steps, (double)samples);
...
*/
    ...
    for(i=0;i<K;i++)
        sum += i*avg_x[i]/n;
    FILE* data = fopen("QueueGeneralChoice.txt", "a");
    fprintf(data, "%f	%f	%f	%f	%f\n", rho, avg_x[0]/n, avg_x[K-1]/n, (float)yij[K-1][K-1]/total_time/n, sum);
    fclose(data);
...
int main(void)
{
    ...
    FILE* data = fopen("QueueGeneralChoice.txt", "w");
    fclose(data);
    data = fopen("QueueGeneralChoice_detailed.txt", "w");
    fclose(data);
    
    for(rho=0.01;rho<4.01;rho+=0.01)
