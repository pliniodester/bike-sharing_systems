...
double next_step_with_hist(int n, int x[], float rho, double hist[K][N+1])
{
    int i, y0=0;
    float aux;
    float u, dt;
...
int run_detailed_system(int n, float rho, int steps, int samples)
{
    ...
    for(i=0;i<samples;i++)
    {
        Dt = next_step_with_hist(n, x, rho, hist);
        total_time += Dt;
        count_yij(n, x, yij);
    }
...
int main(void)
{
    ...
    rho=0.3;
        run_detailed_system(N, rho, 100000, 1000000);
