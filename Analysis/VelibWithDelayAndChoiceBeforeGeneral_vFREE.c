...
*/
    FILE* data = fopen("VelibDelayChoiceFREE.txt", "w");
    fprintf(data, "\nHistogram of the Ny_k samples\n");
    fprintf(data, " -");
    for(i=0;i<n+1;i++)
        fprintf(data, " %d", i);
...
int run_system(int n, int m, int k, float lambda, float mu, int steps, int samples)
{
    ...
    printf("%d %.e %2.f %.2f %.4f(%.4f) ", n, (double)steps+samples, 1/mu, (float)m/n, avg_yk[0]+avg_yk[k], sqrt(dev_yk[0]*dev_yk[0]+dev_yk[k]*dev_yk[k]));
    printf("%.2f %.2f\n",avg_sbikes, (float)m/n-avg_sbikes);
    
    FILE* data = fopen("VelibDelayChoiceNotDetailedFREE.txt", "a");
    fprintf(data, "%d %.e %f %f %f %f ", n, (double)steps+samples, 1/mu, (float)m/n, avg_yk[0]+avg_yk[k], sqrt(dev_yk[0]*dev_yk[0]+dev_yk[k]*dev_yk[k]));
    fprintf(data, "%f %f\n",avg_sbikes, (float)m/n-avg_sbikes);
    fclose(data);
...
/*   
    for(mu=0.1; mu<100; mu=mu*10)
        for(m = 0; m <= N*K; m+=100)
            run_system(N, m, K, LAMBDA, mu, 500000, 500000);
*/   
    getchar();
    return 0;
}
