...
void make_hist(int n, int dx[], long int histp[K][N], long int histn[K][N])
{
    int i, auxp[K], auxn[K];
    set_int_vec(auxp, K);
    set_int_vec(auxn, K);
...
void print_hist(int n, long int histp[K][N], long int histn[K][N], int samples)
{
    int i, j;
    for(i=-K+1;i<0;i++)
    {
...
void count_yij(int n, int x[], long int avg_yij[2*K][2*K])
{
    int i;
    for(i=0;i<n;i++)
    {
        if(x[i]<K && x[i]>-K && x[(i+1)%n]<K && x[(i+1)%n]>-K)
            avg_yij[x[i]+K][x[(i+1)%n]+K]++;
    }
}
...
int run_detailed_system(int n, float dB, int steps, int samples)
{
    ...
    long int histp[K][n], histn[K][n];
    int avgp_x[K], avgn_x[K];
    float devp_x[K], devn_x[K];
    long int avg_yij[2*K][2*K];
    ...
    for(i=0;i<2*K;i++)
    {
        ...
        for(j=0;j<2*K;j++)
        {
            fprintf(data,"%e	", (double)avg_yij[i][j]/samples/n);
        }
    }
    fclose(data);
...
int main(void)
{
    ...
    r=1./3;
        run_detailed_system(N, (1-r)/(3-r), 100000, 10000000);
