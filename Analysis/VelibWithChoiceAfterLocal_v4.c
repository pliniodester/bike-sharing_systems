...
int run_detailed_system(int n, int m, int k, float lambda, float mu, int steps, int samples)
{
    ...
    for(i=0;i<samples;i++)
    {
        ...
        Nvq_yk(hist_sample_vq_yk, bike_station, n);
    }
...
*/
    FILE* data = fopen("VelibWithChoiceAfterLocal_Detailed.txt", "w");
...
int run_system(int n, int m, int k, float lambda, float mu, int steps, int samples)
{
    ...
    FILE* data = fopen("VelibWithChoiceAfterLocal.txt", "a");
...
//    run_detailed_system(N, N*(K/2+LAMBDA/MU), K, LAMBDA, MU, 1000*N, 1000000); //INT_MAX = 2147483647


    FILE* data = fopen("VelibWithChoiceAfterLocal.txt", "w");
    fprintf(data, "STEPS N K lambda/mu s y0+yK var(y0,yK) StationBikes StreetBikes\n");
    fclose(data);
    
    for(mu=1; mu>0.019; mu=mu/(1+5*mu))
        run_system(N, N*(K/2+LAMBDA/mu), K, LAMBDA, mu, 500000, 1000000);
