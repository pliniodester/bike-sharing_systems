...
int run_system(int n, int m, int k, float lambda, float mu, int steps, int samples)
{
    ...
    printf("%d %.1e %2.1f %.2f %.4f(%.4f) ", n, (double)steps+samples, (float)lambda/mu, (float)m/n, avg_yk[0]+avg_yk[k], sqrt(dev_yk[0]*dev_yk[0]+dev_yk[k]*dev_yk[k]));
    printf("%.2f %.2f\n",avg_sbikes, (float)m/n-avg_sbikes);
    
    FILE* data = fopen("VelibDelayChoiceAfterNotDetailed.txt", "a");
    fprintf(data, "%e %d %d %f %f %e %e ", (double)steps+samples, n, k, (float)lambda/mu, (float)m/n, avg_yk[0]+avg_yk[k], sqrt(dev_yk[0]*dev_yk[0]+dev_yk[k]*dev_yk[k]));
    fprintf(data, "%f %f\n",avg_sbikes, (float)m/n-avg_sbikes);
    fclose(data);
...
//    run_detailed_system(N, N*K/2+N*LAMBDA/MU, K, LAMBDA, MU, 1000*N, 100000); //INT_MAX = 2147483647
    ...
    FILE* data = fopen("VelibDelayChoiceAfterNotDetailed.txt", "w");
