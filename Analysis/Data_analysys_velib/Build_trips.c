...
struct bikes_struct* add_to_list(time_t arr_time, int id)
{
    ...
    {
        aux2 = aux1->next;
        while(aux2 != NULL && arr_time < aux2->arr_time)
        {
            aux1 = aux2;
            aux2 = aux1->next;
        }
...
int check_greater(time_t curr_t, state **stations)
{
    ...
    {
        if(mktime(&(stations[i]->lastCheck)) > curr_t)
            return 1;
    }
    return 0;
}
...
int main(void)
{
    ...
    if (trips_data && state_data && results) {
        ...
        for(i=0; i<60000; i++)
        {
            ...
            while(tail!=NULL && tail->arr_time < mktime(&(b_trip->out_date)) )
            {
                if(rel_vec[tail->id] >= 0 && tail->arr_time > mktime(&(current[rel_vec[tail->id]]->lastCheck)) )
                {
            ...
            if(rel_vec[b_trip->out_station]>=0 && mktime(&(b_trip->out_date)) > mktime(&(current[rel_vec[tail->id]]->lastCheck)) )
            {
                current[rel_vec[b_trip->out_station]]->freebk--;
                current[rel_vec[b_trip->out_station]]->freebs++;
            }
            curr_t = mktime(&(b_trip->out_date));
            if(b_trip->out_station==903)
            {
            ...
            while(!(check_greater(curr_t, aux_state)) )
                read_line_state(state_data, aux_state);
            for(i=0; aux_state[i]!=NULL;i++)
            {
                if(mktime(&(aux_state[i]->lastCheck)) > curr_t )
                {
                    current[i]->lastCheck = aux_state[i]->lastCheck;
                    current[i]->freebk = aux_state[i]->freebk;
                    current[i]->freebs = aux_state[i]->freebs;
                }
            }
        }
        
//        fprintf(results, "%s", asctime(localtime(&curr_t)) );
//        print_stations_state(current, results);
