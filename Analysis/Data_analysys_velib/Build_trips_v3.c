...
float eval_perf(state **station)
{
    ...
    for(i=0;station[i]!=NULL;i++)
    {
        ...
        if(station[i]->freebk+station[i]->freebs > 0)
            perf += ( (station[i]->freebk-C_aux)/(station[i]->freebk+station[i]->freebs - C_aux) )*( (station[i]->freebk-C_aux)/(station[i]->freebk+station[i]->freebs - C_aux) );
            j++;
    }
...
int correct_data(state **current, state **aux_state, time_t curr_t, time_t next_t)
{
    int j;
...
int main(void)
{
    ...
    if (trips_data && state_data && results) {
        ...
        for(i=0; i<60000; i++) //60000
        {
            ...
            {
                correct_data(current, aux_state, curr_t, tail->arr_time);
            ...
            correct_data(current, aux_state, curr_t, mktime(&(b_trip->out_date)) );
