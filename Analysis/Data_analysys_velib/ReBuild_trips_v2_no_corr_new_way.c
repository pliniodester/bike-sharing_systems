...
int replay_trip(state **replayed, int close_s[N][15], int rel_vec[], int id)
{
    ...
    aux = rel_vec[id];
    if(aux >= 0)
        max = (float)(replayed[aux]->freebs)/(replayed[aux]->freebs+replayed[aux]->freebk+0.001);
    else
        return 0;
    while(close_s[i][j]!=0)
    {
        if(rel_vec[close_s[i][j]] >= 0 && max < (float)(replayed[rel_vec[close_s[i][j]]]->freebs)/(replayed[rel_vec[close_s[i][j]]]->freebs+replayed[rel_vec[close_s[i][j]]]->freebk+0.001))
        {
            aux = rel_vec[close_s[i][j]];
            max = (float)replayed[aux]->freebs;
        }
        j++;
    }
